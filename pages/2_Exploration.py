...
if sd == "Age and gender":
        ...
        col2.write("South America had the lowest values in every aspect, except losses of over-earners for which Canada had slightly less. Although Canada had the lowest losses in the under-earner population, they had the highest losses in under-earners by twice the amount of Asia (second highest losses)")
        st.write("Note that this graph has **two y-axes**. This is because the distributed capital gains and losses are one order of magnitude different, so only scaling them to different axes would make it possible to view.")
        st.write("The legends of the graphs are next to the corresponding axis it is scalled under. Here, the left axes correspond to the capital gains/losses of over-earners, and the right axis to the gains/losses of under-earners.")
